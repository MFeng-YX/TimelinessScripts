...
class HeadquartersDaily():
    ...
    def city_cal(self, df: pd.DataFrame, category: str) -> pd.DataFrame:
        ...
        city_cal_melt["rank"] = city_cal_melt.groupby("城市线路名称")[category].rank(method="min", ascending=False)
    ...
    def report_production(self, path_list: list[Path]) -> pd.DataFrame:
        ...
        mask_city = cityroute["城市线路名称"].isin(row_set)
        cityroute = cityroute.loc[mask_city, :]    
        ...
        match_col = [col for col in city_match.columns if col not in ["GPT展示日期", "rank"]]
        ...
        report_match = pd.merge(
            gpt_match, city_match,
            how = "left",
            on=["线路名称", "核心影响环节"]
        )
        
        report_match["环比"] = report_match["与第一差值-复盘"] - report_match["与第一差值"]
        ...
        mask_eliminate = (report_match["环比"].isna()) & (~report_match["主要点位"].isna())
        ...
        report = pd.concat([report, city_day.loc[:, col_cityday]], ignore_index=True, axis=0)
