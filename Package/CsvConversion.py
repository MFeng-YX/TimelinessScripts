...
class ExcelToCsv():
    ...
    def __verify_params(self, method: str) -> None:
        ...
        if method not in method_list:
            self.logger.error(f"ExcelToCsv类的method参数没有{method}值, method参数值有: 'dir', 'file'.")
            raise ValueError()
        else:
            return method
    ...
    def to_csv(
        ...
        if self.method == "file":
            ...
            if output_dir == None:
                self.logger.info(f"{self.method}模式下, to_csv方法output_dir参数值不能为None.")
                raise ValueError()
        ...
        self.logger.info(f"读取完成! 耗时: {elapsed:.2f}秒 | 行数: {ws.max_row: ,} , 列数: {ws.max_column: ,}")
    ...
    def __dir_pattern(self) -> list[Path]:
        ...
        input_dir: Path = self.path_exists(input("请输入待转换的Excel文件夹路径:\t").strip().strip("'").strip('"'))
        output_dir: Path = self.path_exists(input("请输入保存Csv文件的文件夹路径: \t").strip().strip("'").strip('"'))
        
        csv_list: list[Path] = list()
        
        for p in input_dir.iterdir():
            csv_path: Path = self.to_csv(p, output_dir=output_dir)
            if csv_path:
    ...
    def __file_pattern(self) -> Path:
        ...
        input_file: Path = self.path_exists(input("请输入待转换的Excel文件路径:\t").strip().strip("'").strip('"'))

        csv_path: Path = self.to_csv(input_file)
    ...
    def operation(self) -> Path | list[Path]:
        """该类的主运行方法
        ...
        if self.method == "dir":
            result: list[Path] = self.__dir_pattern()
